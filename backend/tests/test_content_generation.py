...
def test_extract_text_from_url_rejects_private_network_target() -> None:
    with pytest.raises(ContentExtractionError, match="private or local"):
        extract_text_from_url("http://127.0.0.1:8000/admin")
...
def test_llm_client_retries_once_then_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(LLMClient, "_request_gemini", failing_request)
