...
class LLMClient:
    ...
    def _call_openai_with_retry(self, prompt: str) -> LLMResult:
        ...
        for attempt in range(attempts_allowed):
            try:
                return self._request_openai(prompt, api_key, model)
    ...
    def _request_openai(self, prompt: str, api_key: str, model: str) -> LLMResult:
        try:
            response = httpx.post(
            ...
            raise LLMError(
                f"OpenAI API returned an error ({exc.response.status_code}): {exc}"
            ) from exc
        ...
        return LLMResult(
            ...
            output_tokens=usage.get("completion_tokens", _estimate_tokens(message_content)),
            model=model,
        )
