...
1. После проверки запроса создать строку со статусом `pending`.
2. Перед LLM-вызовом изменить статус на `processing`.
3. После успешной валидации результата записать тексты, usage и статус `completed`.
4. При ошибке записать безопасное `error_message` и статус `failed`.
