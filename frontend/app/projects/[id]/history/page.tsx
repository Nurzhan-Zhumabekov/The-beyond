        ...
        {!loading && !error && items.map((item) => (
          <div key={item.id} className="historyRow">
            ...
            <div className="historyActions">
              <Link href={`/projects/${params.id}/generations/${item.id}`} className="secondary">Open</Link>
              <button className="secondary">Download</button>
              <Link href={`/projects/${params.id}/generate?from=${item.id}`} className="secondary">New variation</Link>
            </div>
          </div>
        ))}
      </section>
