...
Deno.test("generateJson in mock mode returns all required fields without network calls", async () => {
  await withEnv({ LLM_MOCK_MODE: "true" }, async () => {
    ...
    const result = await generateJson(
      "Some ru text about AI content factory",
      failingFetch,
    );
    ...
    assert(
      result.content.key_points.length >= 3 &&
        result.content.key_points.length <= 5,
    );
...
Deno.test("generateJson retries transient failures exactly once (2 attempts total)", async () => {
  await withEnv(
    { LLM_MOCK_MODE: "false", OPENAI_API_KEY: "test-key" },
    async () => {
      let callCount = 0;
      ...
      await assertRejects(
        () => generateJson("prompt", alwaysTimesOut),
        TransientLLMError,
      );
...
Deno.test("generateJson does not retry non-transient errors", async () => {
  await withEnv(
    ...
    async () => {
      ...
      await assertRejects(
        () => generateJson("prompt", badRequestFetch),
        LLMError,
      );
...
Deno.test("generateJson fails fast without an API key configured", async () => {
  await withEnv(
    { LLM_MOCK_MODE: "false", OPENAI_API_KEY: undefined },
    async () => {
