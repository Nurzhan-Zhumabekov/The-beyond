...
function getModel(): string {
  return Deno.env.get("TEXT_MODEL") ?? DEFAULT_MODEL;
}
...
async function requestGemini(
  ...
  try {
    response = await fetchImpl(`${GEMINI_GENERATE_CONTENT_URL}/${model}:generateContent`, {
      method: "POST",
      headers: {
  ...
  return {
    content: parsed,
    inputTokens: typeof usage.promptTokenCount === "number" ? usage.promptTokenCount : estimateTokens(prompt),
    outputTokens:
...
async function callGeminiWithRetry(
  ...
  for (let attempt = 1; attempt <= MAX_TOTAL_ATTEMPTS; attempt++) {
    try {
      return await requestGemini(prompt, apiKey, model, fetchImpl);
    } catch (err) {
      lastError = err;
...
export async function generateJson(
  ...
  return callGeminiWithRetry(prompt, fetchImpl);
}
